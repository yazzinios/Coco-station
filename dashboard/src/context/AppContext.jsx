...
const TOAST_CONFIG = {
  success: { accent: '#2ed573', glow: 'rgba(46,213,115,0.18)', border: 'rgba(46,213,115,0.35)', icon: '✓' },
  error:   { accent: '#ff4757', glow: 'rgba(255,71,87,0.18)',  border: 'rgba(255,71,87,0.35)',  icon: '✕' },
  info:    { accent: '#00d4ff', glow: 'rgba(0,212,255,0.15)',  border: 'rgba(0,212,255,0.3)',   icon: 'ℹ' },
  warning: { accent: '#fd9644', glow: 'rgba(253,150,68,0.18)', border: 'rgba(253,150,68,0.35)', icon: '⚠' },
};
...
function Toast({ toast, onRemove }) {
  ...
  return (
    <div style={{
      position: 'relative', background: 'rgba(18,18,22,0.92)', backdropFilter: 'blur(16px)',
      WebkitBackdropFilter: 'blur(16px)', border: `1px solid ${cfg.border}`,
      borderLeft: `3px solid ${cfg.accent}`, borderRadius: '10px',
      padding: '0.85rem 1rem', boxShadow: `0 8px 32px rgba(0,0,0,0.5), inset 0 0 20px ${cfg.glow}`,
      display: 'flex', alignItems: 'flex-start', gap: '0.75rem',
      minWidth: '300px', maxWidth: '380px', overflow: 'hidden',
      animation: exiting ? 'toastExit 0.28s cubic-bezier(0.4,0,1,1) forwards'
                         : 'toastEnter 0.3s cubic-bezier(0.16,1,0.3,1) forwards',
    }}>
      <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: cfg.glow,
        border: `1px solid ${cfg.border}`, display: 'flex', alignItems: 'center',
        justifyContent: 'center', fontSize: '0.85rem', color: cfg.accent,
        flexShrink: 0, marginTop: '1px', fontWeight: '700' }}>
        {cfg.icon}
      </div>
      <div style={{ flex: 1, fontSize: '0.855rem', lineHeight: '1.45', color: '#e8e8e8', paddingTop: '3px' }}>
        {toast.message}
      </div>
      <button onClick={dismiss} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)',
        cursor: 'pointer', fontSize: '0.95rem', lineHeight: 1, padding: '2px 4px',
        borderRadius: '4px', flexShrink: 0, transition: 'color 0.15s' }}
        onMouseEnter={e => e.currentTarget.style.color = 'rgba(255,255,255,0.7)'}
        onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.3)'}
        aria-label="Dismiss">✕</button>
      <div style={{ position: 'absolute', bottom: 0, left: 0, height: '2px', width: `${progress}%`,
        background: cfg.accent, borderRadius: '0 0 0 10px', transition: 'width 0.03s linear', opacity: 0.7 }} />
    </div>
  );
}
...
function ToastContainer({ toasts, onRemove }) {
  ...
  return (
    <>
      <style>{`
        @keyframes toastEnter { from { opacity:0; transform:translateX(110%) scale(0.95); } to { opacity:1; transform:translateX(0) scale(1); } }
        @keyframes toastExit  { from { opacity:1; transform:translateX(0) scale(1); max-height:120px; } to { opacity:0; transform:translateX(110%) scale(0.95); max-height:0; } }
      `}</style>
      <div style={{ position: 'fixed', bottom: '1.5rem', right: '1.5rem', display: 'flex',
        flexDirection: 'column', gap: '0.65rem', zIndex: 9999, pointerEvents: 'none' }}>
        {toasts.map(t => (
...
export function AppProvider({ children }) {
  ...
  const fetchLibrary = useCallback(async () => {
    try { const r = await authFetch('/api/library'); if (r.ok) setLibrary(await r.json()); } catch { }
  }, [authFetch]);

  const fetchAnnouncements = useCallback(async () => {
    try { const r = await authFetch('/api/announcements'); if (r.ok) setAnnouncements(await r.json()); } catch { }
  }, [authFetch]);

  const fetchPlaylists = useCallback(async () => {
    try { const r = await authFetch('/api/playlists'); if (r.ok) setPlaylists(await r.json()); } catch { }
  }, [authFetch]);

  const fetchRecurringSchedules = useCallback(async () => {
    try { const r = await authFetch('/api/recurring-schedules'); if (r.ok) setRecurringSchedules(await r.json()); } catch { }
  }, [authFetch]);

  const fetchRecurringMixerSchedules = useCallback(async () => {
    try { const r = await authFetch('/api/recurring-mixer-schedules'); if (r.ok) setRecurringMixerSchedules(await r.json()); } catch { }
  }, [authFetch]);

  const fetchSchedulerStatus = useCallback(async () => {
    try { const r = await authFetch('/api/scheduler/status'); if (r.ok) setSchedulerStatus(await r.json()); } catch { }
  }, [authFetch]);
  ...
  const api = {
    ...
    uploadTrack: async (file) => {
      ...
      const r = await authFetch('/api/library/upload', { method: 'POST', body: fd });
      if (!r.ok) throw new Error(await parseError(r));
      await fetchLibrary(); return r.json();
    },
    deleteTrack: async (filename) => {
      const r = await authFetch(`/api/library/${encodeURIComponent(filename)}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
      await fetchLibrary();
    },
    // ── Decks ──
    loadTrack: async (deckId, filename) => {
      const r = await authFetch(`/api/decks/${deckId}/load`, {
    ...
    play:  async (deckId) => {
      const r = await authFetch(`/api/decks/${deckId}/play`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    pause: async (deckId) => {
      const r = await authFetch(`/api/decks/${deckId}/pause`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    stop:  async (deckId) => {
      const r = await authFetch(`/api/decks/${deckId}/stop`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    nextTrack: async (deckId) => {
      const r = await authFetch(`/api/decks/${deckId}/next`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    previousTrack: async (deckId) => {
      const r = await authFetch(`/api/decks/${deckId}/previous`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    loop: async (deckId, loopEnabled) => {
      const r = await authFetch(`/api/decks/${deckId}/loop`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    setVolume: async (deckId, volume) => {
      await authFetch(`/api/decks/${deckId}/volume`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ volume: Math.round(volume) }),
      });
    },
    renameDeck: async (deckId, name) => {
      const r = await authFetch(`/api/decks/${deckId}/name`, {
    ...
    // ── Mic ──
    micOn: async (targets) => {
      const r = await authFetch('/api/mic/on', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    micOff: async () => {
      const r = await authFetch('/api/mic/off', { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    // ── Announcements ──
    createTTS: async (payload) => {
      const r = await authFetch('/api/announcements/tts', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    deleteAnnouncement: async (id) => {
      const r = await authFetch(`/api/announcements/${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
      await fetchAnnouncements();
    },
    updateAnnouncement: async (id, payload) => {
      const r = await authFetch(`/api/announcements/${id}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (!r.ok) throw new Error(await parseError(r));
    ...
    getJingleStatus: async () => {
      ...
      if (!r.ok) return { intro: { exists: false }, outro: { exists: false } };
      return r.json();
    },
    uploadJingle: async (type, file) => {
      const fd = new FormData(); fd.append('file', file);
      const r = await authFetch(`/api/settings/jingles/${type}/upload`, { method: 'POST', body: fd });
      if (!r.ok) throw new Error(await parseError(r));
      return r.json();
    },
    deleteJingle: async (type) => {
      const r = await authFetch(`/api/settings/jingles/${type}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    // ── Playlists ──
    ...
    updatePlaylist: async (id, name, tracks) => {
      const r = await authFetch(`/api/playlists/${id}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, tracks }),
      });
      if (!r.ok) throw new Error(await parseError(r));
    ...
    deletePlaylist: async (id) => {
      const r = await authFetch(`/api/playlists/${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
      await fetchPlaylists();
    },
    loadPlaylist: async (deckId, playlistId, loop = false) => {
      const r = await authFetch(`/api/decks/${deckId}/playlist`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    createMusicSchedule: async (payload) => {
      const r = await authFetch('/api/music-schedules', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (!r.ok) throw new Error(await parseError(r));
    ...
    deleteMusicSchedule: async (id) => {
      const r = await authFetch(`/api/music-schedules/${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    triggerMusicSchedule: async (id) => {
      const r = await authFetch(`/api/music-schedules/${id}/trigger`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    // ── Recurring Schedules ──
    createRecurringSchedule: async (payload) => {
      const r = await authFetch('/api/recurring-schedules', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (!r.ok) throw new Error(await parseError(r));
    ...
    updateRecurringSchedule: async (id, payload) => {
      const r = await authFetch(`/api/recurring-schedules/${id}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (!r.ok) throw new Error(await parseError(r));
    ...
    deleteRecurringSchedule: async (id) => {
      const r = await authFetch(`/api/recurring-schedules/${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    triggerRecurringSchedule: async (id) => {
      const r = await authFetch(`/api/recurring-schedules/${id}/trigger`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    ...
    deleteRecurringMixerSchedule: async (id) => {
      const r = await authFetch(`/api/recurring-mixer-schedules/${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
      await fetchRecurringMixerSchedules();
    },
    triggerRecurringMixerSchedule: async (id) => {
      const r = await authFetch(`/api/recurring-mixer-schedules/${id}/trigger`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    // ── Music Requests ──
    ...
    acceptRequest: async (id) => {
      const r = await authFetch(`/api/requests/${id}/accept`, { method: 'POST' });
      if (!r.ok) throw new Error(await parseError(r));
      return r.json();
    },
    dismissRequest: async (id) => {
      const r = await authFetch(`/api/requests/${id}`, { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
    },
    clearAllRequests: async () => {
      const r = await authFetch('/api/requests', { method: 'DELETE' });
      if (!r.ok) throw new Error(await parseError(r));
    },
